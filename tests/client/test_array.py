...
def test_make_array(loom_env):
    ...
    result_d, result_e0, result_e1, result_e2 = \
        loom_env.submit_and_gather((d, e0, e1, e2))
...
def test_slice_array(loom_env):
    ...
    r0, r1, r2, r3, r4 = \
        loom_env.submit_and_gather((e0, e1, e2, e3, e4))
...
def test_make_empty_array(loom_env):
    a = tasks.array_make(())
    loom_env.start(1)
    result_a = loom_env.submit_and_gather(a)
    assert result_a == []
...
def test_array_of_array(loom_env):
    ...
    loom_env.start(1)
    result_d, result_e, result_f = loom_env.submit_and_gather((d, e, f))
...
def test_array_same_value(loom_env):
    ...
    loom_env.start(1)
    assert [b"ABC"] * 4 == loom_env.submit_and_gather(b)
